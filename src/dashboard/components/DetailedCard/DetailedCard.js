...
import { applyToJob } from '../../../services/contractsService';
...
const DetailedCard = ({ listing, onClose }) => {
  ...
  const handleAction = async () => {
    if (!currentUser) {
      showNotification({
        type: 'error',
        message: t('dashboard.marketplace.loginRequired')
      });
      return;
    }
    ...
    try {
      if (isJob) {
        // Apply to job using contractsService
        ...
        // Contact pharmacist using messagesService
        ...
        const result = await sendMessage(null, { 
          ...
          text: 'I would like to discuss opportunities',
          initialMessage: true
        });
