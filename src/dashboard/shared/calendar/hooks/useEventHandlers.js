...
export const useEventDragDrop = (userId, accountType) => {
  ...
  const handleEventMove = useCallback((eventId, newStartDate, newEndDate, isTemporary = false) => {
    ...
    // Create common structure to store original and new dates for the modal
    ...
    if (isRecurringEvent) {
      showMovemodal(currentEvent, originalEventDatesObj, newEventDatesObj);
      return;
    }
  ...
  const handleEventResize = useCallback((eventId, newStart, newEnd, isTemporary = false) => {
    ...
    if (isRecurringEvent) {
      ...
      showMovemodal(event, originalEventDatesObj, newEventDatesObj);
      return;
    }
...
export const useEventCRUD = (userId, accountType) => {
  ...
  const handleEventSave = useCallback(async (updatedEvent, shouldClose) => {
    ...
    // For recurring events that need modification modal
    if (isModifyingRecurring && shouldClose) {
      // This should be handled by the modification modal
      console.log("Recurring event modification should be handled by modal");
      return;
    }
