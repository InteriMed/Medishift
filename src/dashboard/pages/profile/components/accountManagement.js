  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1200px] mx-auto px-4 py-6">
          ...
          <div className="space-y-3">
            {facilityMemberships.map((facility, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg border border-border/50">
                ...
                <Button
                  onClick={() => { setTargetFacility(facility); setShowLeaveFacilitymodal(true); }}
                  variant="secondary"
                  size="sm"
                >
      ...
      <modal
        ...
        title={t('accountBasics.deleteAccountmodalTitle', 'Delete Account')}
        size="small"
        messageType="warning"
        actions={
          <>
      ...
      <modal
        ...
        onClose={() => { setShowLeaveFacilitymodal(false); setLeaveFacilityConfirmText(''); setTargetFacility(null); }}
        title={t('settings.leaveFacility.modalTitle', 'Leave Facility')}
        size="small"
        messageType="warning"
        actions={
          <>
            <Button onClick={() => { setShowLeaveFacilitymodal(false); setLeaveFacilityConfirmText(''); }} variant="secondary" disabled={isLeavingFacility}>
              {t('common.cancel')}
            </Button>
