...
exports.exportData = onCall(FUNCTION_CONFIG, async (request) => {
...
// Import data
exports.importData = onCall(FUNCTION_CONFIG, async (request) => {
  // Ensure user is authenticated
  if (!request.auth) {
    throw new HttpsError(
      'unauthenticated',
  ...
  try {
    const { userId, importData } = request.data;
...
exports.importFileData = onRequest(async (req, res) => {
  return cors(req, res, async () => {
    try {
      ...
      const result = await exports.importData({
        data: { userId, importData },
        auth: { uid: userId }
      });

      return res.status(200).json(result);
    } catch (error) {
      logger.error('Error in import file data endpoint', error);
